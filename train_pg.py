...
def build_mlp(
    ...
    with tf.variable_scope(scope):
        for i in range(n_layers):
            x = tf.layers.dense(x, size, name='l'+str(i+1), activation=activation)
        out = tf.layers.dense(x, output_size, activation=output_activation, name='out')
    return out 
...
def train_PG(exp_name='',
    ...
    if (len_control is not None):
        ...
        control_params = build_mlp(sy_ob_controller_no, (feature_size+1)*ac_dim, n_layers=n_layers_controller, scope='lol1')
        sy_policy_features_output = build_mlp(sy_ob_no, feature_size, n_layers=n_layers, scope='lol2')
        ...
        sy_policy_output = build_mlp(sy_ob_no, ac_dim, n_layers=n_layers)
    if discrete:
        # YOUR_CODE_HERE
        ...
        while True:
            ...
            while True:
                ...
                if len_control is not None:
                    ...
                    curr_control_len+=1
                    #ipdb.set_trace()
                    ac = sess.run(sy_sampled_ac, feed_dict={sy_ob_no:ob[None], sy_ob_controller_no:control_ob[None]})
                else:
...
def main():
    ...
    parser.add_argument('--hierarchy', '-k', type=int, default=None)
